...
#include "third_party/ThreadLocalHotCache.h"
#include "thread_epoch.h"

// 宏开关定义
// #define USE_HOT_CACHE
// #define USE_RCU_FLAT_SNAPSHOT // <--- 开启/关闭 RCU 平铺快照优化
...
using ThreadLocalCache = ThreadLocalHotCache<CacheEntryComparator>;
extern thread_local uint64_t total_cmp;
extern thread_local uint64_t total_seek;
...
#ifdef USE_RCU_FLAT_SNAPSHOT
// 极度紧凑的数组条目，仅 32 字节，完美契合 CPU Cache Line
struct alignas(32) FlatEntry {
  Key from;
  Key to;
  const CacheEntry* ptr; // 只在查找成功后才使用，比较时不碰它
};
...
#ifdef USE_RCU_FLAT_SNAPSHOT
  std::atomic<FlatSnapshot*> current_snapshot_{nullptr};
  std::list<std::pair<FlatSnapshot*, uint64_t>> snapshot_garbage_list_;
#endif
...
inline const CacheEntry *IndexCache::find_entry(const Key &from,
  ...
  auto before = CacheEntryComparator::cmp_cnt;
  iter.Seek((char *)&e);
  auto after = CacheEntryComparator::cmp_cnt;
  // iter.SeekWithHint((char *)&e, &hint);
  total_cmp += (after - before);
  total_seek++;
  // 检查迭代器是否指向有效的条目
  if (iter.Valid()) {
    // 获取迭代器当前指向的缓存条目
...
#ifdef USE_HOT_CACHE
  CacheSkipList::Node* node = ThreadLocalCache::Instance().Get(k);
  if (node != nullptr) {
...
inline const CacheEntry *IndexCache::find_entry_hot_cache(const Key &from,
  ...
  if (iter.Valid()) {
    // 获取迭代器当前指向的缓存条目
    auto val = (const CacheEntry *)iter.key();
    ThreadLocalCache::Instance().Add(from, iter.GetNode());
    return val;
  } else {
    // 未找到符合条件的条目，返回 nullptr
    return nullptr;
...
#ifdef USE_RCU_FLAT_SNAPSHOT
  // === 极速读路径：全缓存命中的连续数组二分查找 ===
  FlatSnapshot* snap = current_snapshot_.load(std::memory_order_acquire);
  
  if (snap && !snap->entries.empty()) {
    // 逻辑：找到第一个 upper bound (即 'to') 大于等于 k 的条目
    // 因为数组是按照 'to' 递增排序的
    auto it = std::lower_bound(snap->entries.begin(), snap->entries.end(), k,
      [](const FlatEntry& element, const Key& target_key) {
        return element.to < target_key;
      });

    // 如果找到了，并且 k 确实大于等于它的下界，直接命中！
    if (it != snap->entries.end() && k >= it->from && k <= it->to) {
      entry = it->ptr;
    }
  } 
#endif
  
  // 兜底：新数据不在快照中，或者是未开启快照，回退跳表
...
#ifdef USE_RCU_FLAT_SNAPSHOT
    // 2. 清理旧的 Snapshot (使用 delete)
    auto snap_it = snapshot_garbage_list_.begin();
    for (; snap_it != snapshot_garbage_list_.end(); ++snap_it) {
      if (asm_rdtsc() - snap_it->second > 5000ul * 10) {
        delete snap_it->first; // 触发 std::vector 的析构
      } else {
        break;
      }
    }
    snapshot_garbage_list_.erase(snapshot_garbage_list_.begin(), snap_it);

    // 3. 定期构建 RCU 快照
    if (asm_rdtsc() - last_build_time > 5000ul * 1000 * 10) { 
        build_flat_snapshot();
        last_build_time = asm_rdtsc();
    }
#endif
...
#ifdef USE_RCU_FLAT_SNAPSHOT
void IndexCache::build_flat_snapshot() {
  auto new_snapshot = new FlatSnapshot();
  new_snapshot->timestamp = asm_rdtsc();
  ...
  while (iter.Valid()) {
    auto val = (const CacheEntry *)iter.key();
    if (val->ptr != nullptr) {
      new_snapshot->entries.push_back({val->from, val->to, val});
    }
    iter.Next();
  }

  // 替换全局指针
  FlatSnapshot* old_snapshot = current_snapshot_.exchange(new_snapshot, std::memory_order_acq_rel);
  
  // 丢入垃圾回收队列
  if (old_snapshot) {
    snapshot_garbage_list_.push_back(std::make_pair(old_snapshot, asm_rdtsc()));
  }
}
#endif